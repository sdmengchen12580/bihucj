...
public class CaterpillarIndicator extends LinearLayout implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    public CaterpillarIndicator(Context context, AttributeSet attrs) {
        ...
        mTextSizeNormal = a.getDimensionPixelSize(R.styleable.CaterpillarIndicator_slide_text_size_normal, DensityUtil.dip2px(mTextSizeNormal));
        mTextSizeSelected = a.getDimensionPixelSize(R.styleable.CaterpillarIndicator_slide_text_size_selected, DensityUtil.dip2px(mTextSizeNormal));
        mFooterLineHeight = a.getDimensionPixelOffset(R.styleable.CaterpillarIndicator_slide_footer_line_height, DensityUtil.dip2px(2));
        ...
        mItemLineWidth = (int) a.getDimension(R.styleable.CaterpillarIndicator_slide_item_width, DensityUtil.dip2px(22));
    ...
    public void setFooterLineHeight(int mFooterLineHeight) {
        this.mFooterLineHeight = DensityUtil.dip2px(mFooterLineHeight);
        invalidate();
    }
    ...
    public void setItemLineWidth(int mItemLineWidth) {
        this.mItemLineWidth = DensityUtil.dip2px(mItemLineWidth);
        invalidate();
    }
    ...
    public void setTextSizeNormal(int mTextSizeNormal) {
        this.mTextSizeNormal = DensityUtil.dip2px(mTextSizeNormal);
        updateItemText();
    }

    public void setTextSizeSelected(int mTextSizeSelected) {
        this.mTextSizeSelected = DensityUtil.dip2px(mTextSizeSelected);
        updateItemText();
    }
    ...
    //FIXME 修改标题为未选中状态
    public void clearTvContents(int position) {
        switch (position) {
            case 0:
                tvList.get(0).setText("选择省");
                break;
            case 1:
                tvList.get(1).setText("选择市");
                break;
            case 2:
                tvList.get(2).setText("选择区");
                break;
        }
    }

    // FIXME 标题选中状态的文字修改
    public void fixTvContents(int position, String fixContent) {
        tvList.get(position).setText(fixContent);
    }

    // FIXME 判断市是否被选择
    public boolean judgeCityIsNotSelected() {
        return tvList.get(1).getText().toString().trim().equals("选择市");
    }

    // FIXME 判断区是否被选择
    public boolean judgeAreaIsNotSelected() {
        return tvList.get(2).getText().toString().trim().equals("选择区");
    }
