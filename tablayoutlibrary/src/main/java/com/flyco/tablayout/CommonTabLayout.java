...
//                    }
//                }
                //FIXME 第四个按钮不需要判断是否登录
                if (mCurrentTab != position) {
                    if (position != 2 || (position == 2 && !needJudgeFourIconHasLogin)) {
                        setCurrentTab(position);
                        if (mListener != null) {
                            mListener.onTabSelect(position);
                        }
                    ...
                    EventBus.getDefault().post(new TabSelectMyBus(true));
                } else {
                    if (mListener != null) {
                        mListener.onTabReselect(position);
                    }
    ...
    //FIXME 是否需要判断第四项需要登录
    private boolean needJudgeFourIconHasLogin = false;

    public void setBooleanNeedJudge(boolean needJudgeFourIconHasLogin) {
        this.needJudgeFourIconHasLogin = needJudgeFourIconHasLogin;
    }
